      ...
      real*8 R0,C6,alp,R42,disp,y1,z1,x2,y2,z2,rr,e6abc
      real*8 dx,dy,dz,r2,r,r4,r6,r8,r10,r12,t6,t8,t10,damp1
      real*8 damp6,damp8,damp10,e6,e8,e10,e12,tmp1
      real*8 s10,gC6(3),term,step,dispr,displ,r235,tmp2
      real*8 gx1,gy1,gz1,gx2,gy2,gz2,rthr,c8,cn_thr
      real*8 rthr3,s6
      ! zerodamping stuff
      real*8 rs6,rs8
...
c cut-off radii for all element pairs
      ...
      real*8 r2ij,r2jk,r2ik,mijk,imjk,ijmk,rijk3,or
      integer mat,linim,linjm,linkm,kk
      real*8 alp6,alp8,alp10
      ...
      do iat=2,n
        do jat=1,iat-1
          ...
          if (r2.lt.cn_thr) then
            ...
            expterm=exp(-k1*(rcovij*or-1.d0))
            dcn=-k1*rcovij*expterm/(r2*(expterm+1.d0)*(expterm+1.d0))
          else
            dcn=0.d0
          endif
          x1=drij(linij)+dcn*(dc6i(iat)+dc6i(jat))
          g(:,iat)=g(:,iat)+x1*rij*or
          g(:,jat)=g(:,jat)-x1*rij*or
          ! now dCN/dxyz
! dcn(1:3,j,i) : derivative of CN(j) for xyz-displacements of atom i
          dcnij(:,jat,jat)=dcnij(:,jat,jat)+dcn*rij*or
          dcnij(:,iat,jat)=dcn*rij*or
          dcnij(:,jat,iat)=-dcn*rij*or
          dcnij(:,iat,iat)=dcnij(:,iat,iat)-dcn*rij*or
        enddo !iat
      enddo !jat
